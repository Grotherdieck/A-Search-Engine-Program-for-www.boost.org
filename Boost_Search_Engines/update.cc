...
// 数据源
const std::string src_path = "newhtml";
// 放到哪里
...
static void BuildUrl(const std::string& file_name, std::string& url);
...
bool ParseHtml(const std::vector<std::string>& file_list, std::vector<DocInfo_t>& results)
{
    ...
    for (const auto& file : file_list)
    {
        ...
        //     continue;
        // }
        BuildUrl(file, doc.url);
        // 走到这里就完成了解析任务 当前文档的信息已经在doc中
        // 用右值引用 效率更高
        // debug 
        // showdoc(doc);
        // break;
        // if (doc.title == "Router的博客 &#8211; C/C++学习、Linux学习、数学知识、生活感悟分享")
        // {
        //     std::cout << "你添加的网页正在被处理..." << std::endl;
        //     doc.url = "http://www.routeryt.com";
        // }
...
static void BuildUrl(const std::string& file_name, std::string& url)
{
    ...
    url = "http://" + file_name.substr(pos + 1, n - 6 - pos);
...
{
    // 利用ofstream类进行二进制写入
    std::ofstream ofs(output, std::ios::app | std::ios::binary);
    if (!ofs.is_open())
    {
